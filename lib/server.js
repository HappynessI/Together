...
export function dateInTimezone(date = new Date()) {
  return formatDateInTimezone(date, appTimezone());
}
...
export function appTimeMetadata() {
  const now = new Date();
  const timezone = appTimezone();
  return {
    appDate: formatDateInTimezone(now, timezone),
...
export function assertCurrentAppDate(value, {
  code = "DATE_MISMATCH",
  message = "当前日期已经变化，请刷新页面后重试。",
} = {}) {
  const appDate = dateInTimezone();
  const submittedDate = validateDate(value, appDate);
  if (submittedDate !== appDate) {
    throw new HttpError(409, code, message, { appDate, submittedDate });
  }
...
async function resolveMediaUrl(supabase, storedValue) {
  ...
  const cached = signedMediaCache.get(storedValue);
  ...
  if (url) signedMediaCache.set(storedValue, { url, expiresAt: Date.now() + (50 * 60 * 1000) });
  return url;
}
...
export async function loadState(sessionRole) {
  ...
  const [profilesResult, logsResult, walletsResult, wishesResult, reactionsResult] = await Promise.all([
    ...
    readWithRetry("wallet_summary", () => supabase.from("wallet_summary").select("room_id, role_id, points, stars, lifetime_points, earned_stars, spent_stars, points_to_next_star").eq("room_id", ROOM_ID)),
  ...
  const wallets = Object.fromEntries(ROLE_IDS.map((roleId) => [
    `${ROOM_ID}|${roleId}`,
    { lifetimePoints: 0, points: 0, earnedStars: 0, spentStars: 0, stars: 0, pointsToNextStar: 100 },
  ]));
  for (const row of walletsResult.data || []) {
    ...
    wallets[`${row.room_id}|${row.role_id}`] = {
      ...
      earnedStars: Number(row.earned_stars || 0),
      spentStars: Number(row.spent_stars || 0),
      stars: Number(row.stars || 0),
      pointsToNextStar: Number(row.points_to_next_star || 100),
    };
  }
