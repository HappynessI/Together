...
配置完成后重新部署。将 Vercel 生成的网址发给朋友即可：两个人输入各自密码，记录、评分、Stars、愿望、回应和头像/背景都会通过服务端共享。页面每 20 秒轮询一次，并在重新聚焦页面时刷新；如果此时正在编辑或预览，后台同步只更新搭档和进度信息，不会重建编辑器或跳动光标。编辑中的内容先自动保存为本机草稿，点击“保存今日记录”后才同步给对方。
